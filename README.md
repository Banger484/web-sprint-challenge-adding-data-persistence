...
SQL is a query language that can store, manipulate, and retrieve data from a database.
Relational databases are used to store data in tables, they are "relational" because the values in the tables are
all related to each other. Every row is a different data entry, and each column is an attribute that is shared across the entries.

2. Why do tables need a Primary Key?

Primary keys ensure that each row has a unique identifier so we can access the information on any row.

3. What is the name given to a table column that references the Primary Key on another table?

Foreign keys are columns that refer to the primary key of another table.

4. What do we need in order to have a _many to many_ relationship between two tables?

When you are trying to have a many to many relationship you should create a junction table, which would bridge the two tables together by
referencing the primary keys of each table.
